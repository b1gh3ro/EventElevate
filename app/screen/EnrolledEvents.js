...
export default function EnrolledEvents({ navigation }) {
  ...
  useEffect(() => {
    async function fetchEventsData() {
      ...
      for (let i = 0; i < docSnapData.EventsEnrolled.length; i++) {
        console.log("inside the loop");
        console.log(docSnapData.EventsEnrolled[i].trim());
        const docRef = doc(
          FirebaseDB,
          "Events",
          docSnapData.EventsEnrolled[i].trim()
        );
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          console.log("Document data Event:", docSnap.data());
          const dict = docSnap.data();
          dict["id"] = docSnapData.EventsEnrolled[i].trim();
          tempList.push(dict);
        } else {
          // docSnap.data() will be undefined in this case
          console.log("No such document!");
